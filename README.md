...
一个面向双人长期使用、可用 Docker 部署的关系记录网页。当前版本为 `4.0.0`，重点是把“长期记录结构”和“某一阶段的具体矛盾”分开：表单保存事实、感受、需要与回应，阶段目标可以采纳、暂停、完成或归档，不需要关系状态一变化就重新开发数据库。

## 页面与使用方式

- `/journal`：顶层分为“每日记录／本月历史／每周小结／月度复盘”。每日记录再分“小娌记录／小元记录／AI 今日反馈”，每个人的表单分成“事件与感受／沟通与跟进”；手机端日期、查询、导出和模型设置默认折叠，周记录一次只展开一周。
...
返回结果应包含 `"version":"4.0.0"`，且 `gallup_chat`、`period_reviews`、`participant_arrays`、`flexible_records`、`dynamic_actions` 和 `model_selector` 都为 `true`。HTML 已禁用缓存，静态资源也带版本参数；如果健康检查仍不是 `4.0.0`，说明访问的仍是旧容器或旧反向代理目标。
...
页面采用响应式布局：窄屏下导航、模型选择器、操作按钮和 AI 反馈会改为单列；每日记录、周记录、行动清单和盖洛普侧栏通过 Tab 一次只显示当前模块。输入框使用至少 16px 字号，避免 iPhone 聚焦时自动放大；页面不应产生横向滚动。手机访问时使用服务器的局域网 IP，不要使用手机自身的 `127.0.0.1`。
