...
一个面向双人长期使用、可用 Docker 部署的关系记录网页。当前版本为 `5.0.0`。这一版把关系教练扩展为“问答／真实场景复盘／单人练习”三种模式，同时保持真实互动记录、练习技能和现实使用结果相互独立。

## 页面与使用方式

- `/journal`：只有一张按月切换的关系日历，默认选中今天；点击任意日期即可补记，并在同一个日期工作区切换“小娌记录／小元记录／AI 反馈”。周日会出现对应自然周的小结卡片；月度复盘按需从月份标题打开，不再平铺历史或固定周次。
- `/chat`：包含三种关系教练模式。“问答”只回答问题，不评分、不入真实复盘库；“场景复盘”保留原有真实事件分析和双人行为趋势；“单人练习”由 AI 扮演另一方，按步骤卡完成“把事情说小 → 事实＋感受＋需要＋请求 → AI 复述确认 → AI 回应 → 本轮总结”。练习没有计时器，会完整保存到 SQLite 并在刷新后恢复。
...
问答只保存在当前浏览器会话。真实复盘在模型完成结构化分析后，保存问题摘要、关键词、双方期待、才干状态、沟通方案、行为证据、自动评分、进步判断和时间戳。练习则保存完整状态和轮次，但绝不写入真实关系分数。

如果用户在练习完成后点击“我在现实中用过这次表达”，可以主动记录对方实际反应、结果、是否形成约定和是否在暂停后按时回来。统计按稳定策略标签聚合；少于 5 次只显示“3/4 次有帮助”一类原始次数。后续 AI 推荐优先级为：相似且由用户确认有效的现实做法 → 相似真实复盘中的可观察改善 → 盖洛普画像假设 → 通用方法。没有相关历史时不会用最近但无关的记录兜底。
...
返回结果应包含 `"version":"5.0.0"`，`coach_modes` 应为 `qa / review / practice`，且 `practice_persistence`、`practice_step_cards`、`real_world_outcomes` 和 `separate_practice_trends` 都已启用。HTML 已禁用缓存，静态资源也带版本参数；如果健康检查仍不是 `5.0.0`，说明访问的仍是旧容器或旧反向代理目标。
