...
一个面向双人长期使用、可用 Docker 部署的关系记录网页。当前版本为 `5.1.0`。关系 AI 只保留“直接问答／表达练习”两种模式：前者直接给建议，后者教用户看懂原话、换位思考并改写表达。
...
返回结果应包含 `"version":"5.1.0"`，`coach_modes` 应为 `qa / practice`，且 `practice_persistence`、`expression_practice_cards`、`real_world_outcomes` 和 `separate_practice_trends` 都已启用。HTML 已禁用缓存，静态资源也带版本参数；如果健康检查仍不是 `5.1.0`，说明访问的仍是旧容器或旧反向代理目标。
